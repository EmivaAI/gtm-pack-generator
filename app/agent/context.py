...
def build_candidate_context(
    ...
    if candidate.reasons:
        context_lines.append(
            f"Launch Decision Reasons: {json.dumps(candidate.reasons)}"
        )

    if candidate.safety_warnings:
        context_lines.append(
            f"Safety Warnings to Note: {json.dumps(candidate.safety_warnings)}"
        )
    ...
    if brand_profile.allowed_claims:
        context_lines.append(
            f"Allowed Claims: {json.dumps(brand_profile.allowed_claims)}"
        )
    if brand_profile.disallowed_claims:
        context_lines.append(
            f"DISALLOWED Claims (NEVER USE): {json.dumps(brand_profile.disallowed_claims)}"
        )
    ...
    if audience_segments:
        ...
        for seg in audience_segments:
            ...
            if seg.desired_outcomes:
                context_lines.append(
                    f"  - Desired Outcomes: {json.dumps(seg.desired_outcomes)}"
                )
            if seg.objections:
                context_lines.append(
                    f"  - Typical Objections: {json.dumps(seg.objections)}"
                )
            context_lines.append("")
