...
class Workspace(Base):
    ...
    audience_segments: Mapped[List["AudienceSegment"]] = relationship(back_populates="workspace")
...
class ChangeEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("workspaces.id"))
    source_event_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("source_events.id"), unique=True)
    ...
    launch_candidate: Mapped["LaunchCandidate"] = relationship(back_populates="change_event", uselist=False)
...
class LaunchCandidate(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("workspaces.id"))
    change_event_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("change_events.id"), unique=True)
    ...
    status: Mapped[LaunchStatus] = mapped_column(Enum(LaunchStatus), default=LaunchStatus.PENDING_REVIEW)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    change_event: Mapped["ChangeEvent"] = relationship(back_populates="launch_candidate")
    gtm_pack: Mapped["GtmPack"] = relationship(back_populates="launch_candidate", uselist=False)
...
class GtmPack(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    workspace_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("workspaces.id"))
    launch_candidate_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("launch_candidates.id"), unique=True)
    status: Mapped[PackStatus] = mapped_column(Enum(PackStatus), default=PackStatus.DRAFT)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())

    launch_candidate: Mapped["LaunchCandidate"] = relationship(back_populates="gtm_pack")
...
class GtmAsset(Base):
    ...
    audience_id: Mapped[Optional[uuid.UUID]] = mapped_column(ForeignKey("audience_segments.id"), nullable=True)
    ...
    approval_events: Mapped[List["ApprovalEvent"]] = relationship(back_populates="asset")
...
class ApprovalEvent(Base):
    ...
    user_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True)) # Points to your Auth/User table
